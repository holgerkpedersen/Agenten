...
```
agent_core.py         # Agent facade (504 lines): init, tool registration, decompose, execute, thin delegates
agent_tasks.py        # Task execution: solve_task_stream, solve_task, handle_tool_call
agent_tree.py         # Tree operations: parse_tree_from_llm, create_fallback_tree, record_outcome, evolve_if_needed
agent_files.py        # File/chunk operations: read/write/chunk, folder scanning (.env excluded)
agent_skills.py       # Skill matching, template constants (TEMPLATE_TOOLS, get_templates)
...
git_ops.py            # Git + file operations (write_file, edit_file with syntax/dependency/route validation)
...
| `create_issue` | Create a new issue in issues.json |
| `read_issue` | Read an issue from issues.json |
...
| **Overwrite guard** | `.py` files | `write_file` refuses to overwrite existing files — use `edit_file` instead |
...
- **Sessions**: Save/load/rename — persistent JSON storage with atomic write
- **File analysis**: Upload files, folder scanning (.env excluded), automatic chunking
- **Streaming**: Real-time SSE output with thinking toggle
- **Result cascade**: Previous task results fed into next task (sibling cascade removed for independence)
...
Use **🔀 PR Agent** template for automated PR workflow, **💻 Programming task** for code generation, **🖼️ Image analysis** for vision tasks, and **🔧 Refactoring** for code restructuring.