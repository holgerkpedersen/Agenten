...
_HARDCODED_INSTRUCTIONS_DATA = {
    ...
    "bugfix": {
        "Analyse": "TRIN 1: Læs issue med read_issue() — forstå hvad buggen påstår er galt.\nTRIN 2: Find den relevante kode med locate() — brug funktionsnavnet fra issue location.\nTRIN 3: Læs koden med read_location() og sammenlign med buggens påstand.\n  - Er den påståede fejl (f.eks. 'bruger print() i stedet for logging') stadig til stede i koden?\n  - Eller er koden allerede rettet (f.eks. bruger allerede log.warning())?\n  - Hvis koden allerede gør det rigtige: påstanden kan afvises.\nTRIN 4: Hvis påstanden kan afvises (fejlen findes ikke, koden er allerede rettet, eller problemet ikke kan reproduceres), opdater issue-status til 'rejected' med update_issue_status() og skriv en HONEST resolution_note. Afslut med <<<DONE>>>.\nTRIN 5: Hvis fejlen stadig findes, afslut med <<<DONE>>> — resten håndteres i efterfølgende faser.\n\nSKRIV IKKE til filer — kun analyse.",
        "Test (Red)": "Skriv en pytest der verificerer den KORREKTE opførsel — testen skal kalde funktionen med de samme input som buggen beskriver, men ASSERTERE det forventede korrekte output (det funktionen BURDE returnere). Din FØRSTE handling SKAL være write_file for at oprette testfilen i tests/temp/. Kør testen med run_tests(test_path='tests/temp/...') — den SKAL fejle (rød fase). Hvis testen består (i stedet for at fejle), betyder det at testen assertér BUGGY opførsel — omskriv testen så den assertér den korrekte opførsel. Hvis testen stadig består efter omskrivning og koden faktisk allerede gør det rigtige, opdater issue-status til 'resolved'.",
    ...
    "refactor": {
        "Analyse": "⚠️ BUDGET: MAX 12 iterationer i Analyse. Din plan må kun indeholde: (1) list_symbols, (2-5) read_location MAX 4, (6) write_file(refactor_analyse.md), (7) done. Brug IKKE list_files, create_todo, analyze_dependencies, eller update_todo. Notér SOLID-analyse og modul-kortlægning direkte i write_file-indholdet — ikke som separate trin.\n\nTRIN 0 (FØR du læser): Gruppér symbolerne fra list_symbols EFTER NAVNEMØNSTER: `is_*`, `get_*`, `set_*`, `read_*`, `write_*`, `*file*`, `*config*`, `*user*`, `*handler*`, `*process*`, `*route*`, `*import*`, `*extract*`, `*symbol*`, `*dependency*`, `*registry*`, `*modifier*`, `*resolver*`, `*snapshot*` — match grupper til moduler. Dette giver en FØRSTE grovgruppering. Brug read_location til at bekræfte tvivlsomme — match fjerner ledende underscores (_is_X → is_X matcher is_*).\n\nTRIN 1: Kald list_symbols(filepath='{source_file}') og notér ALLE symboler med deres navn og type.\nTRIN 2-5: Læs KUN de 3-4 tvivlsomme funktioner/klasser med read_location() — dem du ikke kunne gruppere 100% sikkert ud fra navnet alene. Spring over de symboler der har et tydeligt navnemønster.\nTRIN 6: Skriv analysen med write_file(path='refactor_analyse.md'). Gem i RODEN (ikke docs/). Analysen SKAL indeholde en komplet liste over ALLE symboler grupperet i moduler — ingen symboler må mangle.\n\n⛔ FORMAT — SKAL bruges:\n\n## Oversigt\n[Overblik]\n\n## Symboler per modul\n\n### Module: config.py\n**Ansvar:** Beskrivelse\n\n| Navn | Type | Linje |\n|------|------|-------|\n| DATABASE_URL | variable | 12 |\n| get_config | function | 25 |\n\nREGLER: ALTID `### Module: file.py` per modul (IKKE 'Modul', IKKE '##'). ALTID tabel med 3 kolonner: Navn, Type, Linje. ALLE symboler SKAL være med.",
        "Plan": "Læs `refactor_analyse.md` med read_chunk() for at få analysen — du behøver IKKE læse symboler/funktioner igen. Beslut modulopdeling og skriv planen med write_file(path='refactor_plan.md', content='...').\n\n⛔ FORMATKRAV — planen SKAL bruge dette EKSAKTE format:\n\n## Oversigt\nKort overblik over modulopdelingen.\n\n## Module: config.py\n**Ansvar:** En-sætnings beskrivelse\n**Symboler (N):** sym1, sym2, sym3, ... (komma-separeret, ÉN linje)\n\n## Module: næste_fil.py\n...\n\nREGLER: ALTID '## Module: file.py' — INGEN backticks omkring filnavnet (forkert: ## Module: `file.py`). ALTID én '**Ansvar:**' linje. ALTID én '**Symboler (N):**' linje med ALLE symboler komma-separeret — INGEN bullets, INGEN backticks omkring symbolnavne.\n\n📋 TRINRÆKKEFØLGE:\n1. Kald plan_phase(fasenavn='Plan', mål='...', steps='...')\n2. Kald read_chunk(path='refactor_analyse.md') for at få analysen\n3. Kald write_file(path='refactor_plan.md', content='...') med planen\n4. Kald <<<DONE>>>\n\n⚠️ DU MÅ KUN returnere tekst NÅR du kalder et værktøj. Hvis du returnerer tekst UDEN værktøj, spilder du iterationer.",
