...
TEMPLATE_TASK_TOOLS = {
    ...
    "issue_handler": {
        ...
        "afklar": ["read_issue", "update_issue_status", "run_tests", "read_location", "read_chunk", "list_chunks", "list_files", "locate", "create_refactor_issue"],
...
SECTION_INSTRUCTIONS = {
    ...
    "bugfix": {
        "Analyse": "Læs issue med read_issue(). Analysér om påstandene i prompten er korrekte eller kan afvises. Hvis issue'et dækker FLERE selvstændige fejl (f.eks. '5 endpoints'), opret da separate issues med create_issue() — ét issue per endpoint/fejl. Forstå koden med read_chunk(). Hvis en påstand kan afvises (fejlen findes ikke, koden er allerede rettet, eller problemet ikke kan reproduceres), opdater issue-status til 'resolved' og afslut. SKRIV IKKE til filer — kun analyse.",
