...
TEMPLATE_PHASE_ITERATION_LIMITS = {
    ...
    "refactor": {
        "Analyse": 8,    # plan_phase + list_symbols + read_location(x5) + write refactor_analyse.md
...
_HARDCODED_INSTRUCTIONS_DATA = {
    ...
    "refactor": {
        "Analyse": "TRIN 1: Brug list_symbols(filepath='{source_file}') for at se ALLE symboler.\nTRIN 2: Brug read_location() på MINDST 5 funktioner/klasser for at forstå struktur, dekoratorer og afhængigheder.\nTRIN 3: Kortlæg ansvarsområder og afhængigheder mellem dem.\nTRIN 4: Identificér SOLID-overtrædelser (SRP: én fil gør for meget).\n📋 Start med at kalde **plan_phase** for at lave en detaljeret plan.\n📝 **SIDSTE HANDLING FØR <<<DONE>>>: Skriv analysen til `refactor_analyse.md` med write_file(path='refactor_analyse.md', content='...')** — Gem i RODEN, ikke docs/.\nOutput: Din analyse som <<<DONE>>> tekst.",
