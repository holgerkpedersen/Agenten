...
TEMPLATE_TOOLS = {
    "resume": ["list_chunks", "read_chunk", "list_files", "locate"],
    "kodeanalyse": ["list_chunks", "read_chunk", "list_files", "locate", "create_issue", "create_refactor_issue"],
    "diffanalyse": ["list_chunks", "read_chunk", "list_files", "locate", "git_diff", "git_log", "create_issue", "create_refactor_issue"],
    "fri": None,
    "agenten": [
        "list_chunks",
        "read_chunk",
    ...
    "programmering": ["list_chunks", "read_chunk", "list_files", "locate", "write_file", "add_image", "create_issue", "create_refactor_issue"],
    "python-arkitektur": ["list_chunks", "read_chunk", "list_files", "locate", "write_file", "create_issue", "create_refactor_issue"],
    "billedanalyse": ["add_image", "write_file", "list_chunks", "read_chunk", "list_files", "locate", "create_issue", "create_refactor_issue"],
    "bugfix": ["read_issue", "update_issue_status", "run_tests", "create_refactor_issue", "create_issue", "list_chunks", "read_chunk", "locate", "write_file", "edit_file", "list_files"],
    "refactor": ["read_issue", "update_issue_status", "list_chunks", "read_chunk", "list_files", "locate", "write_file", "edit_file", "run_tests", "create_issue", "create_refactor_issue"],
    "testgenerering": ["list_chunks", "read_chunk", "list_files", "locate", "write_file", "edit_file", "run_tests", "create_issue", "create_refactor_issue", "update_issue_status"],
    "issue_handler": [
        "read_issue", "update_issue_status", "run_tests",
        "read_chunk", "list_chunks", "list_files", "locate",
...
TEMPLATE_TASK_TOOLS = {
    ...
    "bugfix": {
        "analyse": ["read_issue", "list_files", "list_chunks", "read_chunk", "locate", "run_tests", "create_issue", "create_refactor_issue", "update_issue_status"],
        "test": ["write_file", "list_files", "run_tests", "list_chunks", "read_chunk", "locate", "create_issue", "create_refactor_issue"],
        "implementering": ["edit_file", "list_files", "list_chunks", "read_chunk", "locate", "run_tests", "create_issue"],
        "verifikation": ["run_tests", "edit_file", "list_files", "list_chunks", "read_chunk", "locate", "create_issue"],
        "opdatering": ["update_issue_status", "list_files", "list_chunks", "read_chunk", "locate", "create_issue"],
    },
    "refactor": {
        "analyse": ["read_issue", "list_files", "list_chunks", "read_chunk", "locate", "create_refactor_issue"],
        "plan": ["read_issue", "update_issue_status", "write_file", "list_files", "list_chunks", "read_chunk", "locate"],
        "ekstraher": ["write_file", "list_files", "list_chunks", "read_chunk", "locate"],
        "opdat\u00e9r": ["update_issue_status", "edit_file", "write_file", "list_files", "list_chunks", "read_chunk", "locate"],
        "test": ["update_issue_status", "run_tests", "write_file", "edit_file", "list_files", "list_chunks", "read_chunk", "locate"],
    },
    "testgenerering": {
        "analyse": ["list_files", "list_chunks", "read_chunk", "locate", "run_tests", "create_issue", "create_refactor_issue"],
        "test": ["write_file", "list_files", "run_tests", "list_chunks", "read_chunk", "locate", "create_issue", "create_refactor_issue"],
        "implementering": ["edit_file", "list_files", "list_chunks", "read_chunk", "locate", "run_tests", "create_issue"],
        "verifikation": ["run_tests", "edit_file", "list_files", "list_chunks", "read_chunk", "locate", "create_issue"],
    },
    "issue_handler": {
        "l\u00e6s": ["read_issue", "list_files", "locate"],
        "afklar": ["read_issue", "update_issue_status", "run_tests", "read_chunk", "list_chunks", "list_files", "locate", "create_refactor_issue"],
        "fix": ["read_chunk", "write_file", "edit_file", "run_tests", "list_files", "locate"],
...
SECTION_INSTRUCTIONS = {
    ...
    "refactor": {
        "Analyse": "Læs issue med read_issue(). Læs filen med read_chunk(). Forstå alle funktioner, klasser, imports og deres ansvar. Identificér grænseflader og afhængigheder mellem komponenter. SKRIV IKKE til filer — kun analyse.",
    ...
    "issue_handler": {
        ...
        "læs": "Læs den tildelte issue med read_issue(). Forstå beskrivelse, location, impact og proposed_fix. Du må IKKE læse kildekode eller redigere filer i dette trin. Brug KUN read_issue(). Afslut med <<<DONE>>>.",
        "afklar": "Read issue with read_issue(). Analyze if details are missing — update with update_issue_status() if needed (but do NOT set status='resolved'). Read relevant source code with read_chunk() and run run_tests() to verify if the bug still exists. If already fixed: update status to 'resolved'. End with <<<DONE>>>.",
        "clarify": "Read issue with read_issue(). Analyze if details are missing — update with update_issue_status() if needed (but do NOT set status='resolved'). Read relevant source code with read_chunk() and run run_tests() to verify if the bug still exists. If already fixed: update status to 'resolved'. End with <<<DONE>>>.",
        "fix": "Check issue.type with read_issue(). Fix the bug with edit_file() — use EXACT text from the file. Run run_tests(). Do NOT mark issue as resolved yourself — that is done in Close Issue. End with <<<DONE>>>.",
        ...
        "en_fix": "Check issue.type with read_issue(). Fix the bug with edit_file() — use EXACT text from the file. Run run_tests(). Do NOT mark issue as resolved yourself — that is done in Close Issue. End with <<<DONE>>>.",
        "da_fix": "Tjek issue.type med read_issue(). Ret fejlen med edit_file() — brug PRÆCIS tekst fra filen. Kør run_tests(). Du må IKKE selv markere issue som resolved — det gøres i Luk Issue. Afslut med <<<DONE>>>.",
        "en_clarify": "Read issue with read_issue(). Analyze if details are missing — update with update_issue_status() if needed (but do NOT set status='resolved'). Read relevant source code with read_chunk() and run run_tests() to verify if the bug still exists. If already fixed: update status to 'resolved'. End with <<<DONE>>>.",
