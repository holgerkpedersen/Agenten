...
_HARDCODED_INSTRUCTIONS_DATA = {
    ...
    "refactor": {
        ...
        "Ekstraher": "Opret NYE .py modulfiler med batch_extract_symbols() i.h.t. planen fra forrige fase. 🔥 DETERMINISTISKE VÆRKTØJER: Brug batch_extract_symbols(source='api_server.py', symbols='_RateLimiter, _is_development_mode, _rate_limit', target='middleware.py') for at flytte FLERE symboler i ét kald — systemet kopierer alle symboler inkl. imports, fjerner dem fra api_server.py, OG tilføjer imports i api_server.py. Brug extract_symbol kun hvis batch_extract_symbols fejler, eller til enkelt-symboler. Brug verify_refactor(source='api_server.py') bagefter for at bekræfte syntaks. Du skal oprette FILER som routes.py, session_manager.py, file_handler.py osv. — IKKE write_file til refactor_plan.md (den findes allerede fra Plan-fasen og er auto-indlæst i din kontekst ovenfor). 🔥 EFFEKTIVITETSGUIDE: Du har KUN 15 iterations. Strategi: (1) brug list_symbols FØRST for at se ALLE symboler på én gang, (2) brug batch_extract_symbols til at flytte FLERE symboler i ét kald, (3) brug extract_symbol kun til enkelt-symboler hvis batch fejler, (4) brug verify_refactor hvis du er i tvivl om syntaks. Systemet auto-afslutter denne fase så snart ALLE moduler nævnt i planen er oprettet — du behøver IKKE lave yderligere kald bagefter. 📋 Brug **update_todo(todo_id='lt_xxx', done=true)** for at markere hvert modul når det er oprettet.",
