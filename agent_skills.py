...
_HARDCODED_INSTRUCTIONS_DATA = {
    ...
    "refactor": {
        "Analyse": "TRIN 1: Brug list_symbols(filepath='api_server.py') for at se ALLE symboler.\nTRIN 2: Brug read_location() på MINDST 5 funktioner/klasser for at forstå struktur, dekoratorer og afhængigheder. Identificér hvilke funktioner der har @app.route, @app.before_request osv.\nTRIN 3: Kortlæg ansvarsområder (routes, sikkerhed, sessioner, filhåndtering, billeder, modeller) og afhængigheder mellem dem.\nTRIN 4: Identificér SOLID-overtrædelser (SRP: én fil gør for meget).\n📋 Start med at kalde **plan_phase** for at lave en detaljeret plan med konkrete tool-kald.\n📝 **SIDSTE HANDLING FØR <<<DONE>>>: Skriv din analyse til `docs/refactor_analyse.md` med write_file()** — Plan-fasen og Ekstraher-fasen læser denne fil for at undgå at gentage analysearbejde. Indehold: symboloversigt, ansvarsområder, afhængigheder, SOLID-vurdering og forslag til modulopdeling.\nOutput: Din analyse som <<<DONE>>> tekst.",
        "Plan": "Læs `docs/refactor_analyse.md` med read_chunk() for at få den fulde analyse af filens struktur — du behøver IKKE læse symboler eller funktioner igen, det er allerede gjort i Analyse-fasen. Beslut derefter modulopdeling (f.eks.: routes.py, session.py, files.py, models.py). Brug write_file() til at skrive planen som en .md fil (f.eks. refactor_plan.md). Planen skal indeholde: hvilke moduler der oprettes, hvilke funktioner/klasser der flyttes til hvert modul, og i hvilken rækkefølge.\n📋 Start med at kalde **plan_phase** for at planlægge din fremgangsmåde. Systemet auto-afslutter denne fase så snart refactor_plan.md er skrevet. <<<DONE>>> først når planen er skrevet.",
        "Ekstraher": "Opret NYE .py modulfiler med batch_extract_symbols() i.h.t. planen fra forrige fase. Følg refactor_plan.md TIL PUNKT OG PRIKKE — ALLE moduler nævnt i planen SKAL oprettes. 🔥 DETERMINISTISKE VÆRKTØJER: Brug batch_extract_symbols(source='api_server.py', symbols='_RateLimiter, _is_development_mode, _rate_limit', target='middleware.py') for at flytte FLERE symboler i ét kald — systemet kopierer alle symboler inkl. imports, fjerner dem fra api_server.py, OG tilføjer imports i api_server.py. Brug extract_symbol kun hvis batch_extract_symbols fejler, eller til enkelt-symboler. Brug verify_refactor(source='api_server.py') bagefter for at bekræfte syntaks. Du skal oprette FILER som routes.py, session_manager.py, file_handler.py osv. — IKKE write_file til refactor_plan.md (den findes allerede fra Plan-fasen og er auto-indlæst i din kontekst ovenfor).\n📋 Din opgaveplan (via **plan_phase**) skal indeholde konkrete tool-kald med parametre, f.eks.:\n  `batch_extract_symbols(source='refac_test.py', symbols=['DATABASE_URL', 'MAX_RETRIES', ...], target='config.py')`\n  `verify_refactor(source='refac_test.py')`\nAngiv symbolnavnene eksplicit i hvert trin så rækkefølgen og indholdet er tydeligt.\n🔥 EFFEKTIVITETSGUIDE: Udfør trinene i din plan i rækkefølge. Du har KUN 15 iterations. Systemet auto-afslutter denne fase så snart ALLE moduler nævnt i planen er oprettet. 📋 Brug **update_todo** for at markere hvert modul færdigt.",
        "Opdatér": "Opdater api_server.py med remove_symbol() og add_import() — DETERMINISTISKE værktøjer der ikke kræver LLM. Brug remove_symbol(source='api_server.py', symbol_name='FunktionsNavn') for at fjerne et symbol der allerede er flyttet til et modul. Brug add_import(source='api_server.py', module='routes', symbol='FunktionsNavn') for at tilføje en import til et modul. Brug verify_refactor(source='api_server.py') til at tjekke syntaks.\n📋 Din opgaveplan (via **plan_phase**) bør angive remove_symbol → add_import → verify_refactor for hvert modul. Du har KUN 12 iterations. Brug edit_file KUN hvis remove_symbol/add_import ikke kan klare opgaven. <<<DONE>>> først når api_server.py er syntaktisk gyldig og alle relevante imports er tilføjet.",
