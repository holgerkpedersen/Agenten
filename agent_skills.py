...
_HARDCODED_INSTRUCTIONS_DATA = {
    ...
    "refactor": {
        "Analyse": "TRIN 1: Brug list_symbols(filepath='{source_file}') for at se ALLE symboler.\nTRIN 2: Læs 1-2 funktioner/klasser med read_location() for at forstå strukturen — IKKE alle. Brug de PRÆCISE navne fra list_symbols.\nTRIN 3: Kortlæg ansvarsområder og afhængigheder.\nTRIN 4: Identificér SOLID-overtrædelser.\n📋 Kald **plan_phase(fasenavn, mål)** for at lave en plan. Brug IKKE create_todo/update_todo for hvert trin — plan_phase's steps opretter automatisk todo'er. Udfør planen trin for trin.\n📝 **SIDSTE HANDLING FØR <<<DONE>>>: Skriv analysen til `refactor_analyse.md` med write_file(path='refactor_analyse.md', content='...')** — Gem i RODEN (ikke docs/). Output: <<<DONE>>>.",
