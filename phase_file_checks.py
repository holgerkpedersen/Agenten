"""File-related phase checks: file_exists, files_from_plan, plan parsing helpers."""
...
def _parse_refactor_plan_modules(plan_path: str) -> list[str]:
    ...
    Bruges af _refactor_actually_moved_code til at verificere at ALLE
    moduler fra planen er oprettet under en refactor-session.
...
def _has_real_code(filepath: str, min_lines: int = 20) -> bool:
    ...
    Skelner mellem reel flyttet kode og tomme import-stub.
    Bruges af _refactor_actually_moved_code til at afgøre om en refactor reelt er sket.
...
def _extract_modules_from_plan(
    ...
    modules = parse_modules_from_plan(plan_content, source_file=source_file, allow_nested=allow_nested)
    if ext != ".py":
